		...
		WebApplicationException {
			...
			try {
				...
				while (it.hasNext()) {
					...
					String key = fieldKey.getId();
					List<TagField> fields = tag.getFields(key);
					for (TagField field : fields) {
						if (field instanceof TagTextField) {
							TagTextField text = (TagTextField) field;
							writer.append(String.format("%s -> %s</br>", field.getId(), text.getContent()));
							writer
							.append(String.format("%s -> %s</br>", field.getId(), tag.getSubValue(FieldKey.GENRE, 0, 0)));
						}
						else {
							writer.append(String.format("%s -> %s</br>", field.getId(), field.toString()));
						}
					}
				}
			} catch (Exception e) {
